...
async function main() {
  ...
  await writeFile(join(root, 'js/pricing.data.js'), renderDataFile(computed, config.RATES), 'utf8');

  const htmlPath = join(root, 'index.html');
  const html = await readFile(htmlPath, 'utf8');
  const { html: out, missing } = injectMarkers(html, buildHtmlReplacements(computed));
